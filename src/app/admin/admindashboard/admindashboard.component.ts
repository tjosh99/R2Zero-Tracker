...
export class AdmindashboardComponent implements OnInit {
  ...
  admin_account = this.user.getID(); //undefined
  admin_user = JSON.parse(localStorage.getItem("administrator"));
  userLoggedIn = this.user.getUserLoggedIn();
  ...
  constructor(private ds: DataService, private router: Router, private user: UserService) {
    ...
    if (innerWidth > 992) {
      this.view2 = [innerWidth - 150, 300];
    } else {
      this.view2 = [innerWidth, 300];
    }
  }
  ...
  // onResize(event) {
  //   if (innerWidth > 360) {
  //     this.view = [innerWidth[event] - 400, 300];
  //   } else {
  //     this.view = [innerWidth[event], 300];      
  //   }
  // }
  ...
  private notif_currentdate_all() {
    this.ds.getdata("notif_currentdate_all").subscribe((data: any) => {
      this.currentdate_all = data.payload;
      // console.log(this.currentdate_all);

      this.positive = data.payload[0].currentdate_all;
      this.recover = data.payload[1].currentdate_all;
      this.death = data.payload[2].currentdate_all;
      // console.log(this.positive + this.recover + this.death);
    });
  }
  ...
  leafletMap() {
    for (const property of this.propertyList.payload) {
      ...
      L.circle([property.brgy_lat, property.brgy_lng], { color: 'red', filColor: '#f03', fillOpacity: 0.2, weight: 3, opacity: 0.5, radius: property.brgy_totalcases * 15 / 4 }).addTo(this.mapid)
        ...
        "<br>Total Confirmed: " + property.brgy_totalcases +
        "<br>Total Recovered: " + property.brgy_recoveredcases +
        "<br>Total Death : " + property.brgy_deathcases +
  ...
  confirmedcase() {
    this.ds.push_data("city_cases", null).subscribe((data: any) => {
      let totalcases = data.payload[0].ctotal_cases;
      // console.log(totalcases);

      this.chart1 = new Chart(this.canvas.nativeElement.getContext('2d'), {
        type: 'line',
        data: {
          labels: 0,
          datasets: [{
